...
class ConnectionHandler:
    ...
    def chat(self, query, depth=0):
        ...
        _chat_start_time = _time_mod.time()
        ...
        try:
            ...
            self.logger.bind(tag=TAG).info("[PERF] 预处理耗时: " + str(round(_time_mod.time() - _chat_start_time, 2)) + "s")
        ...
        try:
            ...
            for response in llm_responses:
                if _llm_first:
                    _llm_first = False
                    self.logger.bind(tag=TAG).info("[PERF] LLM 首段文本返回耗时: " + str(round(_time_mod.time() - _chat_start_time, 2)) + "s")
                ...
                if self.intent_type == "function_call" and functions is not None:
                    ...
                    if content is not None and len(content) > 0:
                        ...
                        if "_llm_last_token_time" not in dir():
                            _llm_last_token_time = _llm_token_time
                        _token_delta = _llm_token_time - _llm_last_token_time
                        _llm_last_token_time = _llm_token_time
                        _q = chr(34)
                        _tmsg = "[PERF] LLM token: " + _q + content + _q + " 间隔: " + str(round(_token_delta, 2)) + "s  累计: " + str(round(_llm_token_time - _chat_start_time, 2)) + "s"
                        self.logger.bind(tag=TAG).info(_tmsg)
                        # === PERF END ===
            ...
            self.logger.bind(tag=TAG).info("[PERF] LLM 流式输出结束，总耗时: " + str(round(_time_mod.time() - _chat_start_time, 2)) + "s  累计token数: " + str(len(content_arguments)))
